...
def select_file():
    files = [f for f in os.listdir(app.config["UPLOAD_FOLDER"]) if f.lower().endswith(('.wav', '.mp3', '.flac'))]
...
def analyze():
    if request.method == "POST":
        ...
        csv_path = analyze_transcription_file(transcript_path)  # run analysis on a single file and save CSV
